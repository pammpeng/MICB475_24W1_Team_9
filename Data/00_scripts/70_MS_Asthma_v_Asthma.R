## Purpose: Performing functional analysis (PiCrust2) between the MS+Asthma and Asthma groups
#### Load packages ####
...
metadata <- metadata %>% 
  filter(disease == c('MS_asthma', 'Asthma'))

#Remove NAs for our column of interest
...
# Filter to only include significant pathways and also by log2fold change
sig_res = res_desc %>%
  filter(padj < 0.05,
