...
metadata_sex<- select(metadata, -c("age", "disease_course", "treatment_status", "asthma", "smoke"))
#Remove NAs for your column of interest in this case subject
#metadata_sex = metadata[!is.na(metadata_sex$sex),]

#Filtering the abundance table to only include samples that are in the filtered metadata
sample_names = metadata_sex$'sample-id'
...
metadata_sex = metadata[metadata_sex$`sample-id` %in% abun_samples,] #making sure the filtered metadata only includes these samples
...
abundance_daa_results_df <- pathway_daa(abundance = abundance_data_filtered %>% column_to_rownames("pathway"), 
                                        metadata = metadata_sex, group = "sex", daa_method = "DESeq2")
...
# Generate a heatmap
heatmap<- pathway_heatmap(abundance = abundance_desc %>% column_to_rownames("feature"), metadata = metadata_sex, group = "sex")
heatmap2<- heatmap + theme(axis.text.y = element_text(size = 8)) 
ggsave("heatmap.png", plot = heatmap2, width = 10, height = 6)

# Generate pathway PCA plot
pathway_pca(abundance = abundance_data_filtered %>% column_to_rownames("pathway"), metadata = metadata_sex, group = "sex")
...
res =  DEseq2_function(abundance_data_filtered, metadata_sex, "sex")
