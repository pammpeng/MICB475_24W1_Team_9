...
family 
# Now there are only 38 taxa, compared to 99 before filtering, and there are 75 samples
...
m = model.matrix(~ MS_vs_asthma, data = s)
...
significant_features <- df %>%
  filter(`MS_vs_asthmaMS:pval` < 0.05) %>%
  arrange(desc(abs(`MS_vs_asthmaMS:Est`))) 
...
MS_vs_asthma <- ggplot(significant_taxonomy, aes(x = `MS_vs_asthmaMS:Est`, y = reorder(Genus, `MS_vs_asthmaMS:Est`))) +
  geom_bar(stat = "identity") +  # Plot actual log2 fold change values
  geom_errorbar(aes(xmin = `MS_vs_asthmaMS:Est` - `MS_vs_asthmaMS:SE`, xmax = `MS_vs_asthmaMS:Est` + `MS_vs_asthmaMS:SE`), width = 0.2) +  # Add error bars
  theme_minimal() +  # Clean theme
  coord_flip() +  # Flip coordinates to make it easier to read
  labs(x = "log2 Fold Change", y = "Genus", title = "MS vs Asthma: Significant log2 Fold Changes by Genus") +
...
ggsave(filename="MS vs Asthma.png",MS_vs_asthma)
