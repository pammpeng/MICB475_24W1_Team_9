...
MS_phyloseq1 <- phyloseq(OTU, SAMP, TAX, phylo_tree)

# Let's see how our sequencing depth looks
hist(sample_sums(MS_phyloseq1),breaks = 30) 
...
m = model.matrix(~ MS_asthma_vs_asthma, data = s)
...
significant_features <- df %>%
  filter(`MS_asthma_vs_asthmaMS_asthma:pval` < 0.05) %>%
  arrange(desc(abs(`MS_asthma_vs_asthmaMS_asthma:Est`))) 
...
taxonomy <- tax_table(MS_phyloseq1)
